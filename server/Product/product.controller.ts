...
export async function createProduct(req: Request, res: Response) {
  try {
    ...
    return res
      .status(400)
      .json({ message: error.message || "product not created" });
  }
}
...
export async function searchProduct(req: Request, res: Response) {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "couldnt find products matching name" });
  }
}
...
export async function getTopSellingProducts(req: Request, res: Response) {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "error retrieving top products from database" });
  }
}

export async function getTopSellingProductsByAmount(
  req: Request,
  res: Response,
) {
  const { limit } = req.query;
  ...
  try {
    const products =
      await productService.getTopSellingProductsByDollar(limitNumber);
    return res.status(200).json(products);
  } catch (error) {
    ...
    return res
      .status(500)
      .json({
        message: "error retrieving top products by quantity from database",
      });
  }
}
